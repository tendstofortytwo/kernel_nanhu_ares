...
#define DRIVER_VERSION "aimv7"

#define HARDWARD_VERSION_1  1  //HARRY  3.2
#define HARDWARD_VERSION_2  2  //PORTER 4.0
#define HARDWARD_VERSION HARDWARD_VERSION_2
...
int touch_key_code[] = {KEY_MENU, KEY_HOME, KEY_BACK};
#elif HARDWARD_VERSION==HARDWARD_VERSION_2
int touch_key_code[] = {KEY_MENU, KEY_BACK, KEY_HOME};
#endif
int touch_key_press[] = {0, 0, 0};
unsigned long touch_time=0;

#if HARDWARD_VERSION==HARDWARD_VERSION_2
#define HOME_KEY_GPIO 30
#endif

#define VIRTUAL_KEY_PAD
#define VIRTUAL_FUN_1	1	//0X81 with x position
#define VIRTUAL_FUN_2	2	//0x81 with key_id
...
#define ILI2103t_MAX_COORDINATE_X_VALUE 320
#define ILI2103t_MAX_COORDINATE_Y_VALUE 480

//define key pad range
#define KEYPAD01_X1	0
#define KEYPAD01_X2	106
#define KEYPAD02_X1	106
#define KEYPAD02_X2	212
#define KEYPAD03_X1	212
#define KEYPAD03_X2	320
#define KEYPAD04_X1	320
#define KEYPAD04_X2	320
#define KEYPAD_Y	480//2100
// definitions
	...
	if(i2c.protocol_ver & 0x200){
		...
		for(i=0; i<len; i++){
			...
			if(ilitek_i2c_read(i2c.client, ILITEK_TP_CMD_READ_SUB_DATA, buf, 5)){
				...
				DBG(ILITEK_DEBUG_LEVEL "%s, %X, %d, %d\n", __func__, buf[0], x, y);
			}
		}
		// release point
		if(len == 0){
...
#if HARDWARD_VERSION==HARDWARD_VERSION_2
struct irq_node *home_key_node;
static void 
ilitek_home_key_irq_work_queue_func(
	struct work_struct *work)
{
	struct input_dev *input = i2c.input_dev;
	int homekey_value = gpio_get_value(HOME_KEY_GPIO);
	
	//printk(ILITEK_ERROR_LEVEL "%s Home Key : %d\n", __func__,homekey_value);
	  
	if(homekey_value && (touch_key_hold_press==1))
	{
		if(touch_key_press[2] == 1){
			input_report_key(input, touch_key_code[2], 0);
			touch_key_press[2] = 0;
			touch_key_hold_press = 0;
			//printk("Home Key:%d ID:%d Release\n", touch_key_code[2], 2);
		}
	}
	else if(!homekey_value && (touch_key_hold_press==0 ||touch_key_hold_press==2))
	{
		if(touch_key_press[2] == 0){
			input_report_key(input, touch_key_code[2], 1);
			touch_key_press[2] = 1;
			touch_key_hold_press = 1;
			//printk("Home Key:%d ID:%d press\n", touch_key_code[2], 2);
		}
	}
	//Jordan-20111230 , new function for Qualcomm ICS baseline  
	//set_irq_type(i2c.home_key_irq, homekey_value ? IRQF_TRIGGER_LOW : IRQF_TRIGGER_HIGH);
	//enable_irq(i2c.home_key_irq);
	irq_set_irq_type(i2c.home_key_irq, homekey_value ? IRQF_TRIGGER_LOW : IRQF_TRIGGER_HIGH);
	enable_irq(i2c.home_key_irq);
	//	
}
#endif
	...
	int irq, void *dev_id)
{
	...
	disable_irq_nosync(i2c.home_key_irq);
	queue_work(i2c.irq_work_queue, &i2c.home_key_irq_work);
...
#if HARDWARD_VERSION==HARDWARD_VERSION_2
			     INIT_WORK(&i2c.home_key_irq_work, ilitek_home_key_irq_work_queue_func);

	                   gpio_direction_input(HOME_KEY_GPIO);

	                   gpio_tlmm_config(GPIO_CFG(HOME_KEY_GPIO, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
    
	                   i2c.home_key_irq = MSM_GPIO_TO_INT(HOME_KEY_GPIO);

				if(request_irq(i2c.home_key_irq, ilitek_home_key_isr, IRQF_TRIGGER_LOW, "ilitek_home_key_irq", &i2c))
				{
					printk(ILITEK_ERROR_LEVEL "home key request irq, error\n");
				}
				else{
					printk(ILITEK_ERROR_LEVEL "home key request irq, success, Home Key : %d\n",gpio_get_value(HOME_KEY_GPIO));
				}
#endif			
		}
		else{
